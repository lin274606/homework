...
namespace Work_Calculator
{
    public partial class StudentGrade_List : Form
    {
        ...
        private void btnInsert_Click(object sender, EventArgs e)
        {
            ...
            score.SubItems.Add(statistic[5].ToString());
            score.SubItems.Add(statistic[6].ToString());
            listScore.Items.Insert(0, score);
        ...
        {            
            ...
            {
                listScore.Items.Remove(listScore.SelectedItems[0]);
            }
        }
        ...
        private double GetRangeA()
        {
            double x;
            if (!double.TryParse(txtRangeA.Text, out x) | x < 0 | x > 100)
            {
                MessageBox.Show("請輸入範圍內的數字");
                return double.NaN;
            }
        ...
        private double GetRangeB()
        {
            double y;
            if (!double.TryParse(txtRangeB.Text, out y) | y < 0 | y > 100)
            {
                MessageBox.Show("請輸入範圍內的數字");
                return double.NaN;
            }
        ...
        {            
            double rangeA = GetRangeA();
            double rangeB = GetRangeB();
            if (string.IsNullOrEmpty(txtRangeA.Text) || string.IsNullOrEmpty(txtRangeB.Text)) return;
            if (double.IsNaN(rangeA) || double.IsNaN(rangeB)) return;
            if (rangeA >=rangeB) return;

            int columnToCheck = 1;
            for (int i = listScore.Items.Count - 1; i >= 0; i--)
            {
                ListViewItem item = listScore.Items[i];                
                double value = double.Parse(item.SubItems[columnToCheck].Text);
                if (value < rangeA || value > rangeB)
                {
                    listScore.Items.RemoveAt(i);
                }
